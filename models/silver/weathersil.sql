...
--   json_extract_scalar(_airbyte_data, '$.rain.1h') AS rain_1h,
  ...
  cast(from_unixtime(CAST(json_extract_scalar(_airbyte_data, '$.sys.sunrise') AS DOUBLE)) as timestamp(6) with time zone) AS sunrise,
  cast(from_unixtime(CAST(json_extract_scalar(_airbyte_data, '$.sys.sunset') AS DOUBLE)) as timestamp(6) with time zone) AS sunset,
